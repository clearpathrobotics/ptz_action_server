...
# but not two in either category
    ...
    async def ptz_abs_actionHandler(self, goal_handle):
        ...
        if self.last_axis_state is None:
            ...
            self.goal_tilt =self.clamp(goal_handle.request.tilt, self.min_tilt, self.max_tilt, 'tilt')
            self.goal_zoom = self.clamp(goal_handle.request.zoom, self.min_logical_zoom, self.max_logical_zoom, 'zoom')
    ...
    def vel_actionHandler(self, goal_handle):
        ...
        if self.last_axis_state is None:
            ...
            if cmd.pan != 0 or cmd.tilt != 0 or cmd.zoom != 0:
                ...
                rate = self.create_rate(2)
                timed_out = False
                while self.is_moving and goal_handle.is_active and not goal_handle.is_cancel_requested and not timed_out:
                    feedback = PtzMove.Feedback()
